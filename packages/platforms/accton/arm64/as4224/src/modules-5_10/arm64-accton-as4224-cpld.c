...
static int as4224_cpld_write_internal(struct i2c_client *client, u8 reg,
			u8 value);
...
static SENSOR_DEVICE_ATTR(module_rx_los_all, S_IRUGO, show_rxlos_all, NULL,
	MODULE_RXLOS_ALL);
static SENSOR_DEVICE_ATTR(module_count, S_IRUGO, show_module, NULL,
	MODULE_COUNT);
static SENSOR_DEVICE_ATTR(module_index_begin, S_IRUGO, show_module, NULL,
	MODULE_INDEX_BEGIN);
...
#define MODULE_ATTRS(index) \
	static SENSOR_DEVICE_ATTR(module_present_##index, S_IRUGO, \
		show_module_52x, NULL, MODULE_PRESENT_##index); \
	static SENSOR_DEVICE_ATTR(module_tx_disable_##index, S_IRUGO | S_IWUSR,\
		show_module_52x, set_control_52x, MODULE_TXDISABLE_##index); \
	static SENSOR_DEVICE_ATTR(module_rx_los_##index, S_IRUGO, \
		show_module_52x, NULL, MODULE_RXLOS_##index); \
	static SENSOR_DEVICE_ATTR(module_tx_fault_##index, S_IRUGO, \
		show_module_52x, NULL, MODULE_TXFAULT_##index); \
			 ...
			 char *buf)
{
	...
	u8  size[] = {ARRAY_SIZE(regs_48x), ARRAY_SIZE(regs_52x),
				  ARRAY_SIZE(regs_52x), ARRAY_SIZE(regs_52x)};
	...
	for (i = 0; i < size[data->platform_id]; i++) {
		status = as4224_cpld_read_internal(client,
						regs[data->platform_id][i]);
		if (status < 0)
			goto exit;
			 ...
			 char *buf)
{
	...
	u8 *regs[] = {regs_48x, regs_52x, regs_52x, regs_52x};
	u8 *regs_mask[] = {regs_mask_48x, regs_mask_52x,
			regs_mask_52x, regs_mask_52x};
	u8  size[] = {ARRAY_SIZE(regs_48x), ARRAY_SIZE(regs_52x),
			ARRAY_SIZE(regs_52x), ARRAY_SIZE(regs_52x)};
	struct i2c_client *client = to_i2c_client(dev);
	struct as4224_cpld_data *data = i2c_get_clientdata(client);
	...
	for (i = 0; i < size[data->platform_id]; i++) {
		...
		status = as4224_cpld_write_internal(client,
					regs_mask[data->platform_id][i], 0);
			 ...
			 char *buf)
{
	...
	mutex_lock(&data->update_lock);
	if (attr->index >= MODULE_TXFAULT_1 && 
		attr->index <= MODULE_TXFAULT_48) {
		...
		/* Enable the interrupt to CPU */
		status = as4224_cpld_write_internal(client, reg_mask,
						status & (~mask));
		if (unlikely(status < 0))
			goto exit;
	}
			 ...
			 char *buf)
{
	...
	mutex_lock(&data->update_lock);
	if ((attr->index >= MODULE_TXFAULT_49 && 
		attr->index <= MODULE_TXFAULT_52) ||
		(attr->index >= MODULE_TXDISABLE_49 && 
		attr->index <= MODULE_TXDISABLE_52)) {
		status = as4224_cpld_read_internal(client, 0x36);
		if (unlikely(status < 0))
			goto exit;

		/* Enable the interrupt to CPU */
		status = as4224_cpld_write_internal(client, 0x36, 
						status & (~mask));
		if (unlikely(status < 0))
			goto exit;
	}
...
int as4224_cpld_read(unsigned short cpld_addr, u8 reg)
{
	...
	list_for_each(list_node, &cpld_client_list)
	{
		...
		if (node->client->addr == cpld_addr) {
			ret = as4224_cpld_read_internal(node->client, reg);
			break;
		}
	}
...
int as4224_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
	...
	list_for_each(list_node, &cpld_client_list)
	{
		...
		if (node->client->addr == cpld_addr) {
			ret = as4224_cpld_write_internal(node->client, reg,
							value);
			break;
		}
	}
