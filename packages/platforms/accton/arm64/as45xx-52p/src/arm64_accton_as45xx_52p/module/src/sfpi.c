...
#define MODULE_PRESENT_ALL_ATTR         "/sys/devices/platform/as45xx_sfp/module_present_all"
#define MODULE_RXLOS_ALL_ATTR           "/sys/devices/platform/as45xx_sfp/module_rx_los_all"

int onlp_sfpi_get_port_bus(int port)
{
    as45xx_52p_platform_id_t pid;
    int port_bus_as4500[] = { 13, 14, 15, 16 };
    int port_bus_as4581[] = { 19, 20, 21, 22 };
    int *port_bus[] = { port_bus_as4500, port_bus_as4581 };

    pid = get_platform_id();
    if (pid >= PID_UNKNOWN)
        return -1;

    return port_bus[pid][port-49];
}
...
onlp_sfpi_get_port_attr_int(int port, char *fmt, int *value)
{
    ...
    return onlp_file_read_int(value, "%s%s", SFP_SYSFS_PATH, attr_name);
}
...
onlp_sfpi_set_port_attr_int(int port, char *fmt, int value)
{
    char attr_name[32] = {0};
    sprintf(attr_name, fmt, port);
    return onlp_file_write_int(value, "%s%s", SFP_SYSFS_PATH, attr_name);
}
...
onlp_sfpi_eeprom_read(int port, uint8_t data[256])
{
    ...
    if(onlp_file_read(data, 256, &size, PORT_EEPROM_FORMAT, onlp_sfpi_get_port_bus(port)) != ONLP_STATUS_OK) {
...
onlp_sfpi_dom_read(int port, uint8_t data[256])
{
    ...
    sprintf(file, PORT_EEPROM_FORMAT, onlp_sfpi_get_port_bus(port));
    fp = fopen(file, "r");
    if(fp == NULL) {
...
{
    int bus = onlp_sfpi_get_port_bus(port);
