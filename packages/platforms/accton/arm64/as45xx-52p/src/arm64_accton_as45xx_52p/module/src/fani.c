...
#define FAN_MAX_RPM_FORMAT       "fan%d_max"
...
_onlp_fani_info_get_fan(int fid, onlp_fan_info_t* info)
{
     ...
     */
    ret = onlp_fani_get_fan_attr_int(fid, FAN_MAX_RPM_FORMAT, &value);
    if (ret < 0) {
        return ONLP_STATUS_E_INTERNAL;
    }

    info->percentage = value ? ((info->rpm * 100)/value) : 0;
...
_onlp_fani_info_get_fan_on_psu(int pid, onlp_fan_info_t* info)
{
    ...
    /* get fan direction
     */
    ret = psu_eeprom_str_get(pid, dir, sizeof(dir)-1, "psu_fan_dir");
    if (ret == ONLP_STATUS_OK) {
        if (strncmp(dir, "B2F", strlen("B2F")) == 0)
    ...
    /* get fan speed
     */
    if (psu_pmbus_info_get(pid, "psu_fan1_speed_rpm", &val) == 0) {
        info->rpm = val;
