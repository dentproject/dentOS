...
plat_id_t get_platform_id(void)
{
    ...
    plat_info_t *tn48m_dn_info = &gPlat_info[PID_TN48M_DN];
    plat_info_t *tn4810m_dn_info = &gPlat_info[PID_TN4810M_DN];

    if (plat_os_file_is_existed(tn48m_dn_info->cpld_path)) {
        sprintf(pid_fullpath, "%s/platform_id", tn48m_dn_info->cpld_path);
    } else if (plat_os_file_is_existed(tn4810m_dn_info->cpld_path)) {
        sprintf(pid_fullpath, "%s/platform_id", tn4810m_dn_info->cpld_path);
    } else {
        return PID_UNKNOWN;
    }
    ...
    if (pid >= PID_UNKNOWN || pid < PID_TN48M_DN) {
        return PID_UNKNOWN;
    }
