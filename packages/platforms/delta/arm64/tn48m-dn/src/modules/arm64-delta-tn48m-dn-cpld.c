/*
 * arm64-delta-tn48m-dn-cpld.c - Read/Write tn48m-dn CPLD registers
 *
...
#define TN48M_DN_CPLD_SLAVE_ADDR           0x41

#define TN48M_DN_CPLD_REG_HW_VER           0x00    /* RO */
#define TN48M_DN_CPLD_REG_PLATFORM_ID      0x01    /* RO */
#define TN48M_DN_CPLD_REG_CPLD_VER         0x02    /* RO */
#define TN48M_DN_CPLD_REG_PSU_STATUS       0x0A    /* RO */
#define TN48M_DN_CPLD_REG_SFP_TX_DIS_1     0x31    /* RW */
#define TN48M_DN_CPLD_REG_SFP_PSNT_1       0x3A    /* RO */
#define TN48M_DN_CPLD_REG_SFP_RX_LOS_1     0x40    /* RO */

#define TN48M_DN_CPLD_BIT_PSU1_PRESENT     0
#define TN48M_DN_CPLD_BIT_PSU2_PRESENT     1
#define TN48M_DN_CPLD_BIT_PSU1_PG          2
#define TN48M_DN_CPLD_BIT_PSU2_PG          3

#define I2C_RW_RETRY_COUNT              10
#define I2C_RW_RETRY_INTERVAL           60      /* ms */

#define TN48M_DN_GPIO_I2C_NGPIOS           12
#define TN4810M_DN_GPIO_I2C_NGPIOS         144
...
static struct tn48m_dn_cpld_data *tn48m_dn_cpld_update_device(struct device *dev);

enum tn48m_dn_cpld_sysfs_attributes {
    PSU1_PRESENT,
    PSU2_PRESENT,
    PSU1_PG,
    PSU2_PG,
 ...
 */
static int tn48m_dn_cpld_read_internal(struct i2c_client *client, u8 reg)
{
...
static int tn48m_dn_cpld_write_internal(struct i2c_client *client,
                                     u8 reg, u8 value)
{
    int status = 0, retry = I2C_RW_RETRY_COUNT;
...
int tn48m_dn_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    struct tn48m_dn_cpld_data *data;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ...
            ret = tn48m_dn_cpld_read_internal(cpld_node->client, reg);
...
EXPORT_SYMBOL(tn48m_dn_cpld_read);

int tn48m_dn_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    struct tn48m_dn_cpld_data *data;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            data = i2c_get_clientdata(cpld_node->client);
            mutex_lock(&data->update_lock);
            ret = tn48m_dn_cpld_write_internal(cpld_node->client, reg, value);
            mutex_unlock(&data->update_lock);
            break;
        }
    }
...
EXPORT_SYMBOL(tn48m_dn_cpld_write);
...
static int gpio_i2c_get_value(struct gpio_chip *gc, unsigned int offs)
{
    ...
    struct gpio_i2c_reg *gpio_reg;
    struct tn48m_dn_cpld_data *data = i2c_get_clientdata(chip->i2c);
    int val;
    ...
    mutex_lock(&data->update_lock);
    val = tn48m_dn_cpld_read_internal(chip->i2c, gpio_reg->reg_addr);
    mutex_unlock(&data->update_lock);
...
static void gpio_i2c_set_value(struct gpio_chip *gc, unsigned int offs, int set)
{
    ...
    tn48m_dn_cpld_write_internal(chip->i2c, gpio_reg->reg_addr, val);
    mutex_unlock(&data->update_lock);
}
                               ...
                               struct device_attribute *devattr, char *buf)
{
    ...
    struct tn48m_dn_cpld_data *data = i2c_get_clientdata(client);
    u8 reg_offset;
                             ...
                             struct device_attribute *devattr, char *buf)
{
    ...
    mutex_lock(&data->update_lock);
    reg_data = tn48m_dn_cpld_read_internal(client, data->reg_offset);
    mutex_unlock(&data->update_lock);
                            ...
                            const char *buf, size_t count)
{
    ...
    mutex_lock(&data->update_lock);
    err = tn48m_dn_cpld_write_internal(client, data->reg_offset, reg_data);
    mutex_unlock(&data->update_lock);
...
{
    struct tn48m_dn_cpld_data *data = tn48m_dn_cpld_update_device(dev);
    u8 hw_version;
                                   ...
                                   struct device_attribute *devattr, char *buf)
{
    ...
    switch (attr->index) {
    case PSU1_PRESENT:
        bit = !!(data->psu_status & BIT(TN48M_DN_CPLD_BIT_PSU1_PRESENT));
        break;
    case PSU2_PRESENT:
        bit = !!(data->psu_status & BIT(TN48M_DN_CPLD_BIT_PSU2_PRESENT));
        break;
    case PSU1_PG:
        bit = !!(data->psu_status & BIT(TN48M_DN_CPLD_BIT_PSU1_PG));
        break;
    case PSU2_PG:
        bit = !!(data->psu_status & BIT(TN48M_DN_CPLD_BIT_PSU2_PG));
        break;
    default:
        bit = 0;
                                    ...
                                    const char *buf, size_t count)
{
    ...
    for(i = 0; i < data->sfp_group_num ; i ++) {
        ...
        mutex_lock(&data->update_lock);
        err = tn48m_dn_cpld_write_internal(client,
                                        TN48M_DN_CPLD_REG_SFP_TX_DIS_1 + i,
                                        reg_val);
        mutex_unlock(&data->update_lock);
                                 ...
                                 const char *buf, size_t count)
{
    ...
    mutex_lock(&data->update_lock);
    err = tn48m_dn_cpld_write_internal(client,
                                    TN48M_DN_CPLD_REG_SFP_TX_DIS_1 + group_idx,
                                    reg_val);
    mutex_unlock(&data->update_lock);
...
static struct attribute *tn48m_dn_cpld_attributes[] = {
    ...
    /* tn48m_dn transceiver attributes */
...
static struct attribute *tn4810m_dn_cpld_attributes[] = {
    &sensor_dev_attr_reg_offset.dev_attr.attr,
    &sensor_dev_attr_reg_data.dev_attr.attr,
    &sensor_dev_attr_hw_version.dev_attr.attr,
    &sensor_dev_attr_platform_id.dev_attr.attr,
    ...
    &sensor_dev_attr_psu1_powergood.dev_attr.attr,
    &sensor_dev_attr_psu2_powergood.dev_attr.attr,
    /* tn4810m_dn transceiver attributes */
    &sensor_dev_attr_module_tx_dis_all.dev_attr.attr,
    &sensor_dev_attr_module_present_all.dev_attr.attr,
    &sensor_dev_attr_module_rx_los_all.dev_attr.attr,
    SENSOR_ATTR_SFP_MODULE(1),
...
static const struct attribute_group tn48m_dn_cpld_group = {
    .attrs = tn48m_dn_cpld_attributes,
};

static const struct attribute_group tn4810m_dn_cpld_group = {
    .attrs = tn4810m_dn_cpld_attributes,
};
...
static void tn48m_dn_cpld_add_client(struct i2c_client *client)
{
...
static void tn48m_dn_cpld_remove_client(struct i2c_client *client)
{
    struct list_head    *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int found = 0;
...
static int tn48m_dn_cpld_probe(struct i2c_client *client,
                            const struct i2c_device_id *id)
{
    ...
    data = kzalloc(sizeof(struct tn48m_dn_cpld_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto exit;
    }
    ...
    if (data->type == tn4810m_dn_cpld) {
        /* Read and check the product ID from CPLD */
        product_id = tn48m_dn_cpld_read_internal(client,
                                              TN48M_DN_CPLD_REG_PLATFORM_ID);
        if (product_id != PID_TN4810M_DN) {
            dev_dbg(&client->dev,
        ...
        chip->gpio_chip.ngpio = TN4810M_DN_GPIO_I2C_NGPIOS;
        data->sfp_group_num = 6;

        ret = sysfs_create_group(&client->dev.kobj, &tn4810m_dn_cpld_group);
        if (ret) goto exit_free;
    } else {
        chip->gpio_chip.ngpio = TN48M_DN_GPIO_I2C_NGPIOS;
        data->sfp_group_num = 1;

        ret = sysfs_create_group(&client->dev.kobj, &tn48m_dn_cpld_group);
        if (ret) goto exit_free;
    }

    tn48m_dn_cpld_add_client(client);
...
static int tn48m_dn_cpld_remove(struct i2c_client *client)
{
    struct tn48m_dn_cpld_data *data = i2c_get_clientdata(client);

    tn48m_dn_cpld_remove_client(client);

    /* Remove sysfs hooks */
    if (data->type == tn4810m_dn_cpld)
        sysfs_remove_group(&client->dev.kobj, &tn4810m_dn_cpld_group);
    else
        sysfs_remove_group(&client->dev.kobj, &tn48m_dn_cpld_group);
...
int tn48m_dn_platform_id(void)
{
    int pid = tn48m_dn_cpld_read(TN48M_DN_CPLD_SLAVE_ADDR, TN48M_DN_CPLD_REG_PLATFORM_ID);
    pid &= 0xF;

    if (pid < PID_TN48M_DN || pid > PID_TN4810M_DN) {
        return PID_UNKNOWN;
    }

    return pid;
}
EXPORT_SYMBOL(tn48m_dn_platform_id);
...
static struct tn48m_dn_cpld_data *tn48m_dn_cpld_update_device(struct device *dev)
{
    ...
    if(time_after(jiffies, data->last_updated + HZ) || !data->valid) {
        data->hw_ver =
            tn48m_dn_cpld_read_internal(client, TN48M_DN_CPLD_REG_HW_VER);
        data->platform_id =
            tn48m_dn_cpld_read_internal(client, TN48M_DN_CPLD_REG_PLATFORM_ID);
        data->cpld_ver =
            tn48m_dn_cpld_read_internal(client, TN48M_DN_CPLD_REG_CPLD_VER);

        data->psu_status =
            tn48m_dn_cpld_read_internal(client, TN48M_DN_CPLD_REG_PSU_STATUS);

        for (i = 0; i < data->sfp_group_num; i++) {
            data->sfp_tx_dis[i] =
                tn48m_dn_cpld_read_internal(client,
                                         TN48M_DN_CPLD_REG_SFP_TX_DIS_1 + i);
            data->sfp_present[i] =
                tn48m_dn_cpld_read_internal(client,
                                         TN48M_DN_CPLD_REG_SFP_PSNT_1 + i);
            data->sfp_rx_los[i] =
                tn48m_dn_cpld_read_internal(client,
                                         TN48M_DN_CPLD_REG_SFP_RX_LOS_1 + i);
        }
 ...
 * Driver Data
 */
static const struct i2c_device_id tn48m_dn_cpld_id[] = {
    { "tn48m_dn_cpld", tn48m_dn_cpld },
    { "tn4810m_dn_cpld", tn4810m_dn_cpld },
    { }
};
MODULE_DEVICE_TABLE(i2c, tn48m_dn_cpld_id);

#ifdef CONFIG_OF
static const struct of_device_id tn48m_dn_cpld_of_match[] = {
    {
        .compatible = "dni,tn48m_dn_cpld",
        .data = (void *)tn48m_dn_cpld,
    },
    {
        .compatible = "dni,tn4810m_dn_cpld",
        .data = (void *)tn4810m_dn_cpld,
    },
    {},
};
MODULE_DEVICE_TABLE(i2c, tn48m_dn_cpld_id);
...
static struct i2c_driver tn48m_dn_cpld_driver = {
    .driver = {
        .name  = "arm64_delta_tn48m_dn_cpld",
        .of_match_table = of_match_ptr(tn48m_dn_cpld_of_match),
        .owner = THIS_MODULE,
    },
    .probe     = tn48m_dn_cpld_probe,
    .remove    = tn48m_dn_cpld_remove,
    .id_table  = tn48m_dn_cpld_id,
};

/* I2C init/exit functions */
static int __init tn48m_dn_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&tn48m_dn_cpld_driver);
}

static void __exit tn48m_dn_cpld_exit(void)
{
    i2c_del_driver(&tn48m_dn_cpld_driver);
}

module_init(tn48m_dn_cpld_init);
module_exit(tn48m_dn_cpld_exit);

MODULE_AUTHOR("Chenglin Tsai <chenglin.tsai@deltaww.com>");
MODULE_DESCRIPTION("TN48M_DN/TN4810M_DN I2C CPLD driver");
MODULE_LICENSE("GPL");
