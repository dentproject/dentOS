...
static struct tx4810_dn_cpld_data *tx4810_dn_cpld_update_device(struct device *dev);
...
int tx4810_dn_cpld_read(unsigned short cpld_addr, u8 reg)
{
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = tx4810_dn_cpld_read_internal(cpld_node->client, reg);
            break;
        }
    }
...
int tx4810_dn_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = tx4810_dn_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
                            ...
                            const char *buf, size_t count)
{
    ...
    mutex_lock(&data->update_lock);
    err = tx4810_dn_cpld_write_internal(client, data->reg_offset, reg_data);
    mutex_unlock(&data->update_lock);
                                   ...
                                   struct device_attribute *devattr, char *buf)
{
    ...
    switch (attr->index) {
    case PSU1_PRESENT:
        bit = !!(data->psu_status & BIT(TX4810_DN_CPLD_BIT_PSU1_PRESENT));
        break;
    case PSU2_PRESENT:
        bit = !!(data->psu_status & BIT(TX4810_DN_CPLD_BIT_PSU2_PRESENT));
        break;
    case PSU1_PG:
        bit = !!(data->psu_status & BIT(TX4810_DN_CPLD_BIT_PSU1_PG));
        break;
    case PSU2_PG:
        bit = !!(data->psu_status & BIT(TX4810_DN_CPLD_BIT_PSU2_PG));
        break;
    default:
        bit = 0;
...
{
    int pid = tx4810_dn_cpld_read(TX4810_DN_CPLD_SLAVE_ADDR, TX4810_DN_CPLD_REG_PLATFORM_ID);
    pid &= 0xF;
...
static struct tx4810_dn_cpld_data *tx4810_dn_cpld_update_device(struct device *dev)
{
    ...
    if(time_after(jiffies, data->last_updated + HZ) || !data->valid) {
        data->hw_ver =
            tx4810_dn_cpld_read_internal(client, TX4810_DN_CPLD_REG_HW_VER);
        data->platform_id =
            tx4810_dn_cpld_read_internal(client, TX4810_DN_CPLD_REG_PLATFORM_ID);
        data->cpld_ver =
            tx4810_dn_cpld_read_internal(client, TX4810_DN_CPLD_REG_CPLD_VER);

        data->psu_status =
            tx4810_dn_cpld_read_internal(client, TX4810_DN_CPLD_REG_PSU_STATUS);
