...
struct tx4810_dn_led_data {
	...
	char valid; /* equal to 1 if all gpio are valid */
...
static enum led_light_mode gpio_val_to_led_mode(int led_green, int led_amber)
{
	if (led_green && led_amber) {
		printk (KERN_INFO "Warning: Getting Unknown LED Mode!\n");
...
static int tx4810_dn_gpio_map(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(tx4810_dn_led_gpio); i++) {
		err = gpio_request(tx4810_dn_led_gpio[i].gpio,
				   tx4810_dn_led_gpio[i].name);
		if (err) {
...
static void tx4810_dn_led_update(void)
{
	...
	if (time_after(jiffies, ledctl->last_updated + HZ + HZ / 2)
		...
		/* Update LED data */
		for (i = 0; i < ARRAY_SIZE(tx4810_dn_led_gpio); i++) {
			int status = tx4810_dn_led_read_value(
					tx4810_dn_led_gpio[i].gpio);
				     ...
				     enum led_type type)
{
	...
	tx4810_dn_led_write_value(tx4810_dn_led_gpio[gpio_green_idx].gpio,
				  gpio_green_val);
	tx4810_dn_led_write_value(tx4810_dn_led_gpio[gpio_amber_idx].gpio,
				  gpio_amber_val);
}
...
error:
	for (i = i - 1; i >= 0; i--) {
		/* only unregister the LEDs that in the map */
		if (!(ledctl->led_map & BIT(i))) {
			continue;
		}
